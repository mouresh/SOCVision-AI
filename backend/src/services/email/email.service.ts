...
export class EmailService {
  ...
  private alertHtml(alert: AlertEmailPayload, heading: string): string {
    ...
    return `<div style="font-family:Arial,sans-serif;max-width:600px;background:#0d1117;color:#e6edf3;border-radius:8px">
      ...
      <div style="padding:24px">
        ...
        <p><strong>Severity:</strong> <span style="color:${c};font-weight:bold">${alert.severity?.toUpperCase()}</span></p>
        <p><strong>EventCode:</strong> ${alert.sourceRuleId || 'N/A'}</p>
        <p><strong>Host:</strong> ${alert.host || 'N/A'}</p>
        <p><strong>Source IP:</strong> ${alert.srcIp || 'N/A'}</p>
        <p><strong>Risk Score:</strong> ${alert.riskScore ?? 'N/A'}/100</p>
        <p><strong>Fired:</strong> ${alert.firedAt ? new Date(alert.firedAt as any).toISOString() : 'N/A'}</p>
        ${alert.description ? `<p style="color:#8b949e">${alert.description}</p>` : ''}
